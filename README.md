...
print("Characters:", len(data)) print("Words:", len(data.split())) print("Lines:", data.count("\n")+1)

print("Frequency:", dict(Counter(data))) print("Words reversed:", " ".join(data.split()[::-1]))

open("File1.txt", "w").writelines(data.splitlines()[1::2]) open("File2.txt", "w").writelines(data.splitlines()[::2])

Q10. Class Point :-
class Point: def init(self, x, y): self.x, self.y = x, y def str(self): return f"({self.x},{self.y})" def dist(self, p): return ((self.x - p.x)**2 + (self.y - p.y)**2)**0.5

p1, p2 = Point(1, 2), Point(4, 6) print(p1, p2, "Distance:", p1.dist(p2))
...
print("Half split:", t1[:len(t1)//2], t1[len(t1)//2:]) print("Even values:", tuple(x for x in t1 if x % 2 == 0))

t2 = (11, 13, 15) print("Concatenation:", t1 + t2)

print("Max:", max(t1), "Min:", min(t1))

Q13. Exception Handling :-
try: name = input("Enter your name: ") if not name.isalpha(): raise ValueError("Name must contain only alphabets!") print("Hello,", name) except ValueError as e: print("Error:", e)
...
x = 10 # int y = 3.14 # float z = "Hello" # str
...
for i in range(n): ... while cond: ... break, continue, pass

🔹 5. Functions

def func(a, b=0, *args, **kwargs): return a+b
...
from collections import deque q = deque([1,2]) q.popleft()

✅ Priority Queue (heapq)

Implements min-heap.

import heapq pq = [3, 1, 4] heapq.heapify(pq) heapq.heappop(pq) # smallest
...
try: x = 1/0 except Exception as e: print(e) finally: print("Done")
...
def modify(x): x = x + 10 a = 5 modify(a) print(a) # Output: 5 (unchanged)
...
def modify(lst): lst.append(10) a = [1,2,3] modify(a) print(a) # Output: [1, 2, 3, 10]
...
x = 10 def outer(): x = 20 def inner(): print(x) # Refers to x=20 (enclosing scope, not global) inner() outer()
...
x = 10 def f(): print(x) # Would look up variable in caller’s scope at runtime def g(): x = 20 f() # Would print 20 in dynamic scoping, but prints 10 in Python g()
...
x = 5 def func(): global x x = 10 func() print(x) # Output: 10
...
def func(): y = 100 # local print(y) func() print(y) # Error: y not defined
...
Data Link – Error detection, framing (Ethernet, MAC).

Physical – Hardware, cables, signals, bits.
...
CERN – The Birthplace
In 1989–1990, at CERN (the European Organization for Nuclear Research in Switzerland), a scientist named Tim Berners-Lee proposed a system for sharing research documents over the internet. This system became the World Wide Web.

The Web – The Big Idea
...
Estimate scale → QPS, storage size, peak traffic.

Choose architecture → Monolith / Microservices.

Database design → SQL, NoSQL, sharding, replication.

Caching → Where to reduce latency.

Load balancing → Single point of entry.

Data flow diagram → High-level interactions.
...
Creating VM in AWS
Step-by-Step Guide to Launching a VM (EC2 Instance)​Sign in to the AWS Management Console: First, you need an AWS account. If you don't have one, you can create one. AWS offers a free tier that allows you to use certain resources for a limited time at no cost.
...
​Name and Configure Your Instance:​Name: Give your instance a descriptive name.
...
​Network Settings:​Firewall (Security Group): Configure a security group, which acts as a virtual firewall for your instance. You'll set inbound rules to specify what traffic is allowed to reach your VM (e.g., SSH for Linux or RDP for Windows).

​Configure Storage:​Storage: Specify the size of the root volume (the main disk for the OS). The free tier typically includes up to 30 GB of Amazon Elastic Block Store (EBS) storage.
...
​Connect to Your Instance:​Once the instance status is "running," you can connect to it. Select your instance and click the "Connect" button at the top.

Use of PuTTy in connecting VM
​1. Connect to your instance using PuTTY​To connect, you'll need the Public IPv4 address of your EC2 instance and the .ppk file you created from your private key.

​In PuTTY, enter the Public IPv4 address of your instance in the Host Name (or IP address) field.​Navigate to Connection > SSH > Auth.
...
​2. Install a web server​Once you are connected to the instance's command line, you need to install a web server. The following commands are for Amazon Linux 2.
...
Press Shift 'i'
hello world
Press ctrl+c and then write :wq
Refresh the webpage
...
Copy NS (Name Server) records from Route 53.
...
✅ Verify: docker --version ✅ Test: docker run hello-world
...
Start Minikube (with Docker driver inside AWS VM)
